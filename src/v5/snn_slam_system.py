...
def run_live_slam(key):
    ...
    # ---------------------------------------------------------
    # 🎨 SETUP LIVE PLOTTING (Upgraded to include 1D Ring Plots)
    # ---------------------------------------------------------
    plt.ion()
    fig = plt.figure(figsize=(20, 8))
    gs = fig.add_gridspec(2, 3, width_ratios=[1.5, 1, 1], height_ratios=[2, 1])
    
    ax_map = fig.add_subplot(gs[:, 0]); ax_map.set_title("Phase 3: Real-Time Map")
    
    ax_brain = fig.add_subplot(gs[0, 1]); ax_brain.set_title("Place Cell Memory (2D)")
    ax_cann = fig.add_subplot(gs[0, 2]); ax_cann.set_title("Pose CANN Belief (2D)")
    
    ax_ring_mem = fig.add_subplot(gs[1, 1]); ax_ring_mem.set_title("Ring Visual Memory (1D)")
    ax_ring_cann = fig.add_subplot(gs[1, 2]); ax_ring_cann.set_title("Ring CANN Compass (1D)")
    ...
    # 🌟 NEW: Initialize 1D Line Plots
    x_ring = np.arange(RING_N)
    line_ring_mem, = ax_ring_mem.plot(x_ring, np.zeros(RING_N), color='#9B59B6', lw=2)
    ax_ring_mem.set_ylim(-0.1, 1.1); ax_ring_mem.set_xticks([])
    
    line_ring_cann, = ax_ring_cann.plot(x_ring, np.zeros(RING_N), color='#E67E22', lw=2)
    ax_ring_cann.set_ylim(-0.1, 1.1); ax_ring_cann.set_xticks([])
    ...
    try:
        while True:
            ...
            if step % 15 == 0:
                ...
                ui_smooth_t = 0.15 * raw_t_align + 0.85 * ui_smooth_t
                t_align = ui_smooth_t
                ...
                tr_brain = mtransforms.Affine2D().translate(-MAP_SIZE/2, -MAP_SIZE/2).rotate(delta_theta).translate(MAP_SIZE/2, MAP_SIZE/2) + ax_brain.transData
                brain_img.set_transform(tr_brain)
                
                tr_cann = mtransforms.Affine2D().translate(-CANN_SIZE/2, -CANN_SIZE/2).rotate(delta_theta).translate(CANN_SIZE/2, CANN_SIZE/2) + ax_cann.transData
