...
def run_live_slam(key):
    ...
    MAX_LOOPS = 200
    KEYFRAME_DIST = 0.50     # Add node every xxcm
    KEYFRAME_ANG = 0.50      # Or every xx radians
    ...
    # 🌟 NEW 2: Variable to track the latest HDC search results
    active_candidates = []
                            ...
                            if not abort_lc:
                                if is_cross_path:
                                    ...
                                    if np.sum(valid_mask) >= 2:
                                        ...
                                        # 🌟 INCREASED THRESHOLD: Require strong geometry (corners/angles)
                                        if det_AtA < 0.20: 
