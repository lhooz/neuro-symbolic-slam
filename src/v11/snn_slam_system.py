                ...
                if is_confident[0] and lc_refractory_timer == 0:
                    ...
                    if matched_node is not None:
                        ...
                        if maturity >= LC_MATURITY: 
                            ...
                            # =================================================================
                            # 🌟 THE ALIASING FIX: The Aperture Problem Gate (Anti-Sliding)
                            # =================================================================
                            raw_jump = np.sqrt((cx - matched_x)**2 + (cy - matched_y)**2)
                            tof_diff = np.sum(np.abs(curr_tof - mem_tof))
                            
                            # If CANN believes we moved far, but lasers see no depth difference,
                            # we are sliding along a featureless flat wall! ABORT!
                            if raw_jump > 0.40 and tof_diff < 0.20:
                                print(f"\n ⚠️ Aborted: Aperture Problem! Sliding on flat wall. Jump={raw_jump:.2f}m, ToF Diff={tof_diff:.2f}m")
                                matched_node = None
                            ...
                            # =================================================================
                            # 🌟 THE BUG FIX: Bridging the Discrete SNN and Continuous Graph
                            # =================================================================
                            
                            # 1. Calculate the SNN's raw sub-pixel visual rotation
                            sub_pixel_th = float(np.clip(-pixel_shift * pixel_ang_res, -0.30, 0.30))
                            
                            # 2. Extract the winning Ring Cell ID to get the Absolute Macro-Angle
                            r_idx = int(debug_gates['Peak_Ring'][0])
                            ring_angle = float(r_idx) * (2.0 * np.pi / 64.0)
                            
                            # 3. Calculate the True Absolute Compass Heading according to Vision
                            vision_th = float(wrap_angle(ring_angle + sub_pixel_th))
                            
                            # 4. Calculate the True Crossing Angle for the Graph Optimizer
                            # (The difference between Vision's absolute heading and the Node's historical heading)
