...
from snn_vision_fusion import DualStreamVisionCortex
from snn_pose_cann import (
    PoseCANN,
...
DRIFT_START     = 7500     # (Offline Default) step at which drift kicks in
                ...
                if is_confident[0]:
                    ...
                    if place_id in place_to_node:
                        for nid in place_to_node[place_id]:
                            if current_node_id - nid > 10:  # Prevent snapping to immediate history
                                ...
                                if dth < 0.30 and dth < min_dth:
                                    min_dth = dth
                                    matched_node = nid

                    if matched_node is not None:
                        # 🌟 NEW: Hard Security Cutoff!
                        maturity = float(debug_gates['Maturity_Lvl'][0])
                        if maturity < 0.85:
                            continue # The brain is not confident enough. Abort!
                            
                        matched_x, matched_y, matched_th = graph_poses[matched_node]
                        
                        # 🌟 UPGRADE 1: True Loop Closure (Resting length is zero)
                        # We assume a match means we are in the exact same footprint as before.
                        lc_offset_x = 0.0
                        lc_offset_y = 0.0
                        lc_offset_th = 0.0
                        
                        # 🌟 UPGRADE 2 & 3: Variance Modulation & Soft Nudges
                        maturity = float(debug_gates['Maturity_Lvl'][0])
                        conc_p   = float(debug_gates['Conc_Place'][0])
                        conc_r   = float(debug_gates['Conc_Ring'][0])
                        
                        # Max stiffness matches the Odometry springs (0.20 pos, 0.15 th).
                        # We modulate this by both historical Maturity AND current Vision Sharpness (Concentration).
                        ...
                        # 2. Filter Closures & Summon Frozen Anchors
                        ...
                        for i in range(len(recent_lcs)):
                            ...
                            if current_id < start_idx: 
                                continue # This closure belongs to the frozen past. Ignore it.
                                
                            mapped_current = current_id - start_idx
                            
                            if target_id >= start_idx:
                                # Internal Spring: Both nodes are inside the active window
                                mapped_target = target_id - start_idx
                            else:
                                # Anchor Spring: The memory is deep in the frozen past!
                                # Summon the historical coordinates and append them as a ghost.
                        ...
                        # 🌟 THE SEVERING FIX: Tell physics where the active chain stops!
                        odom_mask = np.zeros(MAX_NODES - 1, dtype=np.float32)
                        if num_active > 1:
                            odom_mask[:num_active-1] = 1.0
                            
                        # 4. Build the is_frozen Mask
                        is_frozen = np.zeros(MAX_NODES, dtype=bool)
                        is_frozen[0] = True # Always freeze the tail of the active window!
                        if frozen_anchors:
                        ...
                        # 7. Update Global Map (Only update the active nodes, leaving past untouched)
