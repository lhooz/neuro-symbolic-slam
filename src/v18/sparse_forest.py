...
def _make_trajectory(key, time_steps, dt, obstacles=None):
    ...
    for t in range(1, time_steps):
        ...
        # 🌟 THE OMNIDIRECTIONAL FIX: Introduce Holonomic Slip!
        v_slip = rng.uniform(VY_RANGE[0], VY_RANGE[1]) if rng.uniform() > 0.6 else 0.0
            
        h = hdg[t-1] + current_omega * dt
        px = pos[t-1, 0] + (v_forward * np.cos(h) - v_slip * np.sin(h)) * dt
        py = pos[t-1, 1] + (v_forward * np.sin(h) + v_slip * np.cos(h)) * dt
