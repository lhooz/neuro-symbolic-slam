...
def relax_graph(poses, odom_edges, loop_closures, loop_offsets, loop_weights, loop_mask, iterations=3000):
    """
    3DOF Force-directed graph relaxation (X, Y, Theta).
    Now upgraded with Relative Transforms and Dynamic Confidence Weights!
    """
    k_odom_pos = 0.20
    k_odom_th = 0.15 
    damping = 0.85 
    ...
    def step_fn(i, state):
        ...
        p_new_x = p_new_x.at[0].set(poses[0, 0])
        p_new_y = p_new_y.at[0].set(poses[0, 1])
        p_new_th = p_new_th.at[0].set(poses[0, 2]) 
...
class SNNSLAMSystem:
    ...
    def phase_mapping(self, dual_vis_features, tof_features, pose_bump, ring_bump, angular_vel):
        vis_csnn, vis_stdp = dual_vis_features
        self.place_state, _ = self.place.forward_mapping(
            self.place_state, vis_csnn, vis_stdp, tof_features, pose_bump, ring_bump=ring_bump, angular_vel=angular_vel, learn=True)
    ...
    def forward_step(self, events_t, kin_t, tof_t, inject_drift=False):
        ...
        # 🌟 WE KEEP THIS: Pass the raw IMU angular velocity down for Saccadic Masking!
        r_place, r_ring = self.phase_mapping(dual_vis_features, tof_features, pose_bump, ring_bump, kin_t[:, 2])
...
def run_live_slam(key):
    ...
    # 🌟 Collapse from 3 columns to 2 columns
    gs = fig.add_gridspec(2, 2, width_ratios=[1.5, 1], height_ratios=[2, 1])
    
    ax_map = fig.add_subplot(gs[:, 0]); ax_map.set_title("Phase 3: Real-Time Map")
    ...
    # Use 'cool' (cyan/magenta) for the CANN so it pops against the Magma memory!
    cann_img = ax_2d.imshow(np.zeros((CANN_SIZE, CANN_SIZE)), cmap='cool', origin='lower', vmin=0.1, vmax=1.0, alpha=0.7)
    ...
    ui_smooth_t = np.zeros(2)
    history['live_offsets'] = []  # 🌟 ADD THIS HERE
    try:
        while True:
            ...
            if is_keyframe:
                ...
                if current_node_id == 0:
                    ...
                    # We can reuse the local offsets we just calculated!
                ...
                # 🌟 THE FIX: Always extract the Place ID, even if we aren't confident yet!
                # This ensures the dictionary is populated for future loop closures.
                place_id = int(peak_idx_place[0])

                if is_confident[0]:
                    # 🌟 THE CONJUNCTIVE ORCHESTRATOR FIX
                    # Find the historical node at this Place that ALSO matches our Heading!
                    matched_node = None
                    min_dth = 999.0
                    
                    if place_id in place_to_node:
                        for nid in place_to_node[place_id]:
                            if current_node_id - nid > 20:  # Prevent snapping to immediate history
                                _, _, nth = graph_poses[nid]
                                # Calculate shortest angular distance
                                dth = abs(wrap_angle(cth - nth))
                                
                                # Only accept the node if it was facing the same way! (e.g., < 0.6 rad difference)
                                if dth < 0.60 and dth < min_dth:
                                    min_dth = dth
                                    matched_node = nid
                    ...
                    if matched_node is not None:
                        ...
                        # 🌟 SOLUTION 1: Relative Transforms (Offsets)
                        # 1. Calculate raw global difference
                        raw_dx = cx - matched_x
                        raw_dy = cy - matched_y
                        
                        # 2. Rotate it into the local frame of the historical node
                        lc_offset_x = raw_dx * np.cos(-matched_th) - raw_dy * np.sin(-matched_th)
                        lc_offset_y = raw_dx * np.sin(-matched_th) + raw_dy * np.cos(-matched_th)
                        
                        # 3. Calculate angular difference
                        lc_offset_th = wrap_angle(cth - matched_th)
                        
                        # 🌟 SOLUTION 2: Biological Confidence-Weighted Springs
                        maturity = float(debug_gates['Maturity_Lvl'][0])
                        scale = max(0.0, min(1.0, (maturity - 0.50) / 0.50))
                        w_pos = 0.05 + scale * (0.80 - 0.05)  
                        w_th  = 0.05 + scale * (0.60 - 0.05)  
                        ...
                        if num_lc > 0:
                            ...
                            lc_offsets_padded[:num_lc] = np.array(loop_offsets_list[-num_lc:])
                            lc_weights_padded[:num_lc] = np.array(loop_weights_list[-num_lc:])
                            lc_mask[:num_lc] = 1.0
                        ...
                        relaxed_p = relax_graph(
                            p_jax, o_jax, 
                            jnp.array(lc_padded), 
                            jnp.array(lc_offsets_padded), 
                            jnp.array(lc_weights_padded), 
                            jnp.array(lc_mask)
                        )
                ...
                # 🌟 IMPORTANT: Append the node to the list for future loop closures!
                # Note how this is now outside the if is_confident block, so it always runs.
                place_to_node[place_id].append(current_node_id)
            ...
            # 🌟 V4.3 FIX B: Reconstruct the ultra-smooth live trajectory!
            smooth_cl_poses = []
            for i, nid in enumerate(history['frame_to_node']):
                rx, ry, rth = graph_poses[nid]
                ldx, ldy, ldth = history['live_offsets'][i]
                
                # Rotate local offset by the RELAXED graph heading
                gdx = ldx * np.cos(rth) - ldy * np.sin(rth)
                gdy = ldx * np.sin(rth) + ldy * np.cos(rth)
                
                smooth_cl_poses.append(np.array([rx + gdx, ry + gdy, (rth + ldth + np.pi) % (2*np.pi) - np.pi]))
            ...
            if step % 15 == 0:
                ...
                # It will overwrite itself seamlessly, showing you exactly what the brain is feeling!
                print(f"\r--- Step {step:04d} | Snap: {snap} | VisAct: {vis_act:.2f} | Match: {match_val:.2f} | Conc(P/R): {c_place:.2f}/{c_ring:.2f} | Mat: {maturity:.2f} ---        ", end="", flush=True)
                ...
                raw_delta_th = gt_th - rel_cth
                angle_diff = (raw_delta_th - ui_smooth_th + np.pi) % (2*np.pi) - np.pi
                ui_smooth_th += 1.0 * angle_diff  
                # delta_theta = ui_smooth_th
                delta_theta = 0.0
                ...
                raw_t_align = np.array([gt_pos[0], gt_pos[1]]) - R_align @ np.array([rel_cx, rel_cy])
                ui_smooth_t = 1.0 * raw_t_align + 0.0 * ui_smooth_t
                # t_align = ui_smooth_t
                t_align = np.array([0.0, 0.0])
                ...
                # 🌟 Hide the empty CANN space so the Memory shines through!
                cann_masked = np.where(cann_norm > 0.15, cann_norm, np.nan)
                cann_img.set_data(cann_masked)
