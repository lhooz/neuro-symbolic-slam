...
TAU_TRACE = 100.0       # 100 ms decay time constant     
...
class STDPLayer:
    ...
    def apply_batch(self, W, e_pre, e_post, spike_pre_all, spike_post_all):
        ...
        # 🌟 THE HOMEOSATIC REVOLUTION: Activity-Dependent Synaptic Scaling (L1 Normalization)
        # Instead of a brutal time-based global decay that drains weights to zero over time,
        # we maintain the total incoming synaptic weight sum constant for each hidden neuron.
        # This keeps weights perfectly stable during idle periods, while introducing standard
        # homeostatic competition when new features are learned.
...
class VisionSTDP:
    ...
    def __call__(self, state: VisionSTDPState, time_surface, tof_dist, learn=True):
        ...
        # 🌟 FIX: Normalize by per-sample max so feature scale stays bounded [0, 1]
