...
def run_live_slam(key):
    ...
    # 🌟 Split the top-right panel into a 2-row, 3-column mini grid!
    gs_top_right = gs[0, 1].subgridspec(2, 3, height_ratios=[0.5, 1])
    
    ax_place = fig.add_subplot(gs_top_right[0, :])
    ax_place.set_title("Place Memory (579-dim Grid Key)", fontsize=10)
    ax_place.set_yticks([])
    
    ax_cann1 = fig.add_subplot(gs_top_right[1, 0])
    ax_cann2 = fig.add_subplot(gs_top_right[1, 1])
    ax_cann3 = fig.add_subplot(gs_top_right[1, 2])
    ax_cann1.set_title("3.0m Wrap", fontsize=9); ax_cann1.axis('off')
    ax_cann2.set_title("4.0m Wrap", fontsize=9); ax_cann2.axis('off')
    ax_cann3.set_title("5.3m Wrap", fontsize=9); ax_cann3.axis('off')
    ...
    # 🌟 Plot Place Memory Barcode
    brain_img = ax_place.imshow(np.zeros((1, N_PLACE)), cmap='magma', aspect='auto', vmin=0, vmax=1.0)
    
    # 🌟 Plot all 3 CANN Modules (No transparency needed anymore!)
    ...
    try:
        while True:
            ...
            if is_keyframe:
                ...
                # 🌟 THE GHOST DATABASE FIX: File the new keyframe into the SNN's biological memory folder!
                place_to_node[recalled_place_id].append(current_node_id)

                # 🌟 NEW: Track if the loop closure survives the physics gates
                lc_success = False
                ...
                if is_confident[0] and lc_refractory_timer == 0:
                    ...
                    if matched_node is not None:
                        ...
                        if maturity >= LC_MATURITY: 
                            ...
                            # Initialize the gate control flag
                            abort_lc = False
                            ...
                            if not abort_lc:
                                ...
                                # 2. Obstacle Limit: If the perpendicular error is > 0.20m, it hit a 
                                ...
                                if np.sum(valid_mask) >= 2:
                                    ...
                                    if not abort_lc:
                                        ...
                                        # =================================================================
                                        # 🌟 THE BUG FIX: Extract the 3 independent Grid Cell peaks!
                                        i_place_flat = np.array(debug_gates["Debug_I_Place"][0])
                                        s1, s2 = CANN_SIZES[0]**2, CANN_SIZES[1]**2
                                        
                                        p1_idx = int(np.argmax(i_place_flat[:s1]))
                                        p2_idx = int(s1 + np.argmax(i_place_flat[s1:s1+s2]))
                                        p3_idx = int(s1 + s2 + np.argmax(i_place_flat[s1+s2:]))
                                        ...
                                        # 🌟 Update ALL THREE pendulums sequentially!
                                        for target_p_idx in [p1_idx, p2_idx, p3_idx]:
                                            system_cl.place_state = system_cl.place.apply_post_relaxation_update(
                                                system_cl.place_state,
                                                jnp.array([target_p_idx]), 
                                                jnp.array([r_idx]),
                                        ...
                                        print(f"  ↳ 🧠 Phase 2 Plasticity: Applied 5% Hebbian update to M1({p1_idx}), M2({p2_idx}), M3({p3_idx}) with a {pixel_shift_int}px mental rotation!")
                                        ...
                                        # 🌟 NEW: Mark the attempt as a complete success!
                                        lc_success = True
                                        ...
                                        # You accidentally deleted this block during the merge!
                                        # ==================================================
                                        MAX_NODES = 1000
                                        WINDOW_SIZE = 800
                                        ...
                                        if "Debug_Input_CSNN" in debug_gates:
                                            ...
                                            s1_ui, s2_ui = CANN_SIZES[0]**2, CANN_SIZES[1]**2
                                            p1_ui = np.argmax(i_place[:s1_ui])
                                            p2_ui = s1_ui + np.argmax(i_place[s1_ui:s1_ui+s2_ui])
                                            p3_ui = s1_ui + s2_ui + np.argmax(i_place[s1_ui+s2_ui:])
                                            
                                            axs[3, 0].axvline(p1_ui, color='gold', linestyle='--', lw=2, label=f"M1 ({p1_ui})")
                                            axs[3, 0].axvline(p2_ui, color='cyan', linestyle='--', lw=2, label=f"M2 ({p2_ui})")
                                            axs[3, 0].axvline(p3_ui, color='magenta', linestyle='--', lw=2, label=f"M3 ({p3_ui})")
                                            axs[3, 0].set_title("Spatial Soma Activation (3 Grid Peaks)", fontweight='bold')
                                ...
                                # 🌟 THE INDENTATION CORRECTION FOR THE FALLBACK GATES 🌟
                                else:
                ...
                # ==================================================
                # If we attempted a loop closure but it failed the physics gates:
                if is_confident[0] and not lc_success:
                    
                    # 1. The Abort Penalty: Force the system to sleep for 10 frames (0.5s)
                    # This forces the robot to physically move away from the confusing visual alias!
                    lc_refractory_timer = 10 
                    
                    # 2. Automatic Pruning: If the robot barely moved (Micro-Node), delete the garbage node!
                    if dist < KEYFRAME_DIST and ang < KEYFRAME_ANG and current_node_id > 0:
                        ...
                        # Erase it from the physics graph
                        graph_poses.pop()
                        graph_odom_edges.pop()
                        node_tof_hits.pop()
                        
                        # Erase it from the biological database
                        place_to_node[recalled_place_id].remove(current_node_id)
                        
                        # Rewire the UI anchor back to the previous valid node
                        last_kf_cann = (float(graph_poses[-1][0]), float(graph_poses[-1][1]), float(graph_poses[-1][2]))
                        
                        # Pretend this keyframe never happened so the UI rubber-band continues smoothly!
                        is_keyframe = False

            # --- Outside the keyframe block ---       
            # 🌟 THE FIX: Zero out the lingering offset if we just anchored a new node!
            if is_keyframe:
            ...
            if step % 15 == 0:
                ...
                # Update 1D Barcode for Place Memory
                pc_act = np.array(r_place[0]).reshape(1, N_PLACE)
                brain_img.set_data(pc_act / (pc_act.max() + 1e-8))
                
                # 🌟 Update all 3 CANN Modules!
                s1, s2 = CANN_SIZES[0]**2, CANN_SIZES[1]**2
