...
TOFPOP_ANTI_ALIAS_POS = 0.60   
TOFPOP_ANTI_ALIAS_HEA = 0.70  
MATCH_THRESH          = 0.50   
MATURITY_GATE         = 0.70   
MATCH_TOF             = 0.80
...
class PlaceCellNetwork:
    ...
    def _updateHebbian(self, state: PlaceCellState, vis_csnn, vis_stdp, tof_features, pose_bump, ring_bump, heading, angular_vel=None, confidence=None):
        ...
        spatial_novelty_mask = jnp.where(spatial_match_ratio < 0.90, 1.0, 0.0)[:, None, None]
    ...
    def compute_confidence_with_gates(self, state: PlaceCellState, vis_csnn, vis_stdp, tof_features, pose_bump, heading, ring_bump):
        ...
        # 🌟 THE FIX: Clamp the match score to 1.0 so it never exceeds 100% confidence
        recalled_place = jnp.minimum(recalled_place / (max_sensory_energy + 1e-8), 1.0)
