...
DRIFT_OMEGA     = 0.010  # rad/s artificial yaw drift per timestep
...
class SNNSLAMSystem:
    ...
    def phase_odometry(self, kin_t, I_gated_place, I_gated_ring, inject_drift=False):
        ...
        pose_est = self.pose(
            ...
            map_correction_ring=I_gated_ring * 0.2,  # ring gain 20% of place (0.002 effective)
        )
...
def evaluate_system(key, n_samples=BATCH_SIZE, time_steps=TIME_STEPS,
    ...
    for t in range(T):
        ...
        # Events are too sparse (0.8% rate → most frames blank), so show
        # the actual continuous intensity image the sensor sees instead
...
def main():
    ...
    TIME_STEPS  = 1500   # bumped from 500; B-spline supports long looping trajectories
    DRIFT_START = 80
