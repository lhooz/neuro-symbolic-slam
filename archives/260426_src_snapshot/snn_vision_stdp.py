...
def lateral_inhibition(spike, k=K_WTA):
    B, N = spike.shape
    _, topk_idx = jax.lax.top_k(spike, k)  
...
class STDPLayer:
    ...
    def apply_batch(self, spike_pre_all, spike_post_all):
        e_pre_expanded = jnp.broadcast_to(self.e_pre[None, :], spike_pre_all.shape)  
        e_post_expanded = jnp.broadcast_to(self.e_post[None, :], spike_post_all.shape) 
...
class VisionSTDP:
    ...
    def __call__(self, ev_frame, tof_dist, learn=True):
        B = ev_frame.shape[0]
        ev_pol = self.polarize_events(ev_frame) 
        
        i_syn = ev_pol @ self.stdp.W.T 
        i_ext = i_syn
        ...
        # 3. Lateral Inhibition (Only the top K are allowed to fire)
