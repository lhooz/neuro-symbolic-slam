...
TOFPOP_ANTI_ALIAS_POS = 0.60   # top-16 concentration threshold for anti-aliasing for each cann module
TOFPOP_ANTI_ALIAS_HEA = 0.65   # top-8 concentration threshold for anti-aliasing
MATCH_THRESH          = 0.65   # Minimum fraction of theoretical max energy (0.0 - 1.0) for a valid match
MATURITY_GATE         = 0.75   # Hard security cutoff for loop closure readiness
MATCH_TOF             = 0.85   # TOF match threshold 
...
class PlaceCellNetwork:
    ...
    def compute_confidence_with_gates(self, state: PlaceCellState, vis_csnn, vis_stdp, tof_features, pose_xy, heading, ring_bump):
        ...
        # 1. Slice the 579-dim flat memory back into its 3 modules (11^2, 13^2, 17^2)
        s1, s2 = 121, 169
        p1 = place_norm[:, :s1]
        p2 = place_norm[:, s1:s1+s2]
        p3 = place_norm[:, s1+s2:]

        # 2. A clean 2D Gaussian bump is roughly 3x3 neurons (9 total). 
        # We grab the top 16 from each module to safely capture the full peak.
        top16_m1 = jax.lax.top_k(p1, 16)[0].sum(axis=1)
        top16_m2 = jax.lax.top_k(p2, 16)[0].sum(axis=1)
        top16_m3 = jax.lax.top_k(p3, 16)[0].sum(axis=1)

        # 3. Calculate concentration for each module individually!
        # (We divide the distractor by 3 since the energy is now split 3 ways)
        distractor = VIRTUAL_DISTRACTOR_ENERGY / 3.0
        conc_m1 = top16_m1 / (p1.sum(axis=1) + distractor)
        conc_m2 = top16_m2 / (p2.sum(axis=1) + distractor)
        conc_m3 = top16_m3 / (p3.sum(axis=1) + distractor)

        # Average them for the Terminal UI printout so you can track it cleanly
        conc_place = (conc_m1 + conc_m2 + conc_m3) / 3.0

        top8_ring   = jax.lax.top_k(ring_norm, 8)[0].sum(axis=1)
        conc_ring   = top8_ring / (ring_norm.sum(axis=1) + VIRTUAL_DISTRACTOR_ENERGY)

        # 4. The Biological Gate: ALL THREE modules must be sharp and un-aliased!
        is_place_anti_aliased = jnp.where(
            has_learned_memory, 
            (conc_m1 > TOFPOP_ANTI_ALIAS_POS) & 
            (conc_m2 > TOFPOP_ANTI_ALIAS_POS) & 
            (conc_m3 > TOFPOP_ANTI_ALIAS_POS), 
            True
        )
        ...
        winning_W_tof_ring  = state.W_tof_to_ring[batch_idx, peak_idx_place, :, peak_ring]
        # 🌟 NEW: Grab the exact continuous heading burned into this specific memory!
        winning_theta_burn = state.theta_burn[batch_idx, peak_idx_place, peak_ring]

        debug_gates = {
            "Peak_Ring": peak_ring,
            "Peak_Theta_Burn": winning_theta_burn, # 🌟 The Orchestrator will use this!
