...
TOFPOP_ANTI_ALIAS_POS = 0.30   # top-16 concentration threshold for anti-aliasing
TOFPOP_ANTI_ALIAS_HEA = 0.25   # top-4 concentration threshold for anti-aliasing
...
MATCH_TOF             = 0.80   # TOF match threshold 
...
class PlaceCellNetwork:
    ...
    def compute_confidence_with_gates(self, state: PlaceCellState, vis_csnn, vis_stdp, tof_features, pose_xy, heading, ring_bump):
        ...
        # 🌟 THE FIX: The theoretical max energy no longer expects Sequence/Inertia!
        ...
        # Prevents false 1.0 concentration when the map is mostly empty.
        # =================================================================
        VIRTUAL_DISTRACTOR_ENERGY = 2.0 

        # 🌟 THE FIX: Normalize the arrays so the peak is always exactly 1.0!
        # This prevents raw synaptic energy (500,000+) from swallowing the 2.0 distractor.
        ...
        top16_place = jax.lax.top_k(place_norm, 16)[0].sum(axis=1)
        conc_place  = top16_place / (place_norm.sum(axis=1) + VIRTUAL_DISTRACTOR_ENERGY)

        top4_ring   = jax.lax.top_k(ring_norm, 4)[0].sum(axis=1)
        conc_ring   = top4_ring / (ring_norm.sum(axis=1) + VIRTUAL_DISTRACTOR_ENERGY)
