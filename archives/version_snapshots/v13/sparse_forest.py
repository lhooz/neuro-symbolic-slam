...
MAX_ROOM_ATTEMPTS = 100   # max room regenerations per sample
MAX_TRAJ_ATTEMPTS = 50   # max trajectory tries per room

# Texture
TEX_FREQS = [0.5, 1.0, 2.0]  # 🌟 REDUCED: Slower frequencies for wider gradients
...
def _make_trajectory(key, time_steps, dt, obstacles=None):
    ...
    for t in range(1, time_steps):
        ...
        if rng.uniform() < 0.02: # 2% chance per frame to change steering
            # 🌟 FIX: Strictly bind wandering to OMEGA_RANGE
        ...
        # 4. Evasive Maneuvers!
        if hit:
            # 🌟 FIX: Pivot hard, but DO NOT exceed OMEGA_RANGE limits!
...
def _barcode_texture(barcode_key, local_coords):
    ...
    # 🌟 REDUCED: Creates 3 to 8 large, distinct panels instead of 15-45 tiny stripes
