    ...
    try:
        while True:
                                ...
                                if is_cross_path:
                                    ...
                                    if np.sum(valid_mask) >= 2:
                                        ...
                                        if not abort_lc:
                                            ...
                                            # 🌟 THE ALIASING FIX: Verify the shift explains ALL in-range rays
                                            ...
                                            # If ICP cannot mathematically explain a ray's difference, it's a False Positive!
                                            if np.any(in_range & (residuals > 0.20)):
            ...
            if step % 15 == 0:
                ...
                if conf and maturity >= LC_MATURITY:
                    status = "🟢 LOCKED"
                elif conf:
                    status = "🟡 MATURING"
                elif is_learning:
                ...
                print(f"\r--- Step {step:04d} | Status: {status:10s} | VisAct: {vis_act:.2f} | P_Match: {match_val:.2f} | ToFMatch: {tof_score:.2f} | P_Conc: {c_place:.2f} | R_Conc: {c_ring:.2f} | Mat: {maturity:.2f} ---        ", end="", flush=True)
                ...
                # Paths remain in their raw coordinate frames
                aligned_cl_path = cl_arr[:, :2]
                aligned_ol_path = ol_arr[:, :2]
                # No rotation offset applied to the heading or map
                delta_theta = 0.0
