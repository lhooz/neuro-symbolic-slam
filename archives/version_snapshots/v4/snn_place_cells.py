...
GATING_STRENGTH = 0.10  # STDP Multiplier power

# SOTAv1 gating thresholds
TOFPOP_ANTI_ALIAS_POS = 0.80   # top-16 concentration threshold for anti-aliasing
TOFPOP_ANTI_ALIAS_HEA = 0.95   # top-4 concentration threshold for anti-aliasing
HEADING_THRESH        = 0.60
RING_SELF_MATCH_THRESH = 0.04  # Radians
PLAUSIBILITY_THRESH   = 3.00
SELF_MATCH_THRESH     = 0.05   # meters — restored: blocks "molasses effect" backward corrections
...
class PlaceCellNetwork:
    ...
    def _updateHebbian(self, state: PlaceCellState, vis_csnn, vis_stdp, tof_features, pose_bump, ring_bump, confidence=None):
        ...
        raw_dW_sp = eta_batch * jnp.einsum('bs,bp->bsp', new_trace_stdp, new_trace_place)
        ...
        raw_dW_sr = eta_batch * jnp.einsum('bs,br->bsr', new_trace_stdp, new_trace_ring)
