...
CORRECTION_SMOOTH    = 0.15  # temporal smoothing of correction signal
RING_CORRECTION_SMOOTH = 0.10
GATING_STRENGTH = 0.0  # STDP Multiplier power
...
TRACE_ALPHA = 0.10   # slow decay: xx% carry-over, 1-xx% new input
...
class PlaceCellNetwork:
    ...
    MATCH_THRESH     = 0.10  # Seed threshold
    ...
    def __call__(self, state: PlaceCellState, vis_csnn, vis_stdp, tof_features, pose_bump, ring_bump, angular_vel=None, learn=True, confidence=None):
        ...
        # This is the 2D Landscape: P(Place, Heading | Vision)
        joint_ring = jnp.maximum(0.0, joint_csnn_ring * (1.0 + GATING_STRENGTH * joint_stdp_ring) * joint_tof_ring)
    ...
    def _updateHebbian(self, state: PlaceCellState, vis_csnn, vis_stdp, tof_features, pose_bump, ring_bump, angular_vel=None, confidence=None):
        ...
        mask_place_4d = jnp.where(pose_bump > 0.05, 1.0, 0.0)[:, :, None, None]
        mask_ring_4d  = jnp.where(ring_bump > 0.05, 1.0, 0.0)[:, None, None, :]
