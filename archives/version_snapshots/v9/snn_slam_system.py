...
class SpikingOccupancyGrid:
    ...
    def __init__(self, map_size_m=30.0, res=0.10, offset_m=10.0, v_max=None): # 🌟 UPDATED
        ...
        self.beta = 0.9998        # Leak rate (0.98 = forgets unconfirmed hits slowly)
...
class SNNSLAMSystem:
    ...
    def phase_mapping(self, dual_vis_features, tof_features, pose_bump, ring_bump, angular_vel):
        ...
        # 🌟 THE FIX: Unpack the visual recall (I_place, I_ring) directly from the tuple!
                ...
                if is_confident[0]:
                    ...
                    if matched_node is not None:
                        ...
                        if maturity >= LC_MATURITY: 
                            ...
                            # =================================================================
                            # 🌟 STEP 1: SOLVE ROTATION (Windowed 1D Cross-Correlation)
                            # =================================================================
                            input_csnn = np.array(debug_gates["Debug_Input_CSNN"][0])
                            mem_csnn_ring = np.array(debug_gates["Debug_Mem_CSNN_Ring"][0])
                            
                            # 🌟 THE FIX: Do NOT mean-center sparse neuromorphic arrays!
                            # Correlate the raw edges so the empty space evaluates to zero!
                            correlation = np.correlate(input_csnn, mem_csnn_ring, mode='full')
                            
                            center_idx = len(input_csnn) - 1
                            
                            # 🌟 THE FIX: The Search Window
                            # We know the Orchestrator blocks anything > 0.30 rads (~17 degrees).
                            # 17 degrees is exactly 24 pixels in a 256-pixel / 180-FOV camera.
                            # We force the algorithm to IGNORE hallucinations outside this physical limit!
                            search_radius = 24
                            window_start = center_idx - search_radius
                            window_end = center_idx + search_radius + 1
                            
                            # Extract just the physically possible window
                            windowed_corr = correlation[window_start:window_end]
                            
                            # Find the peak INSIDE the safe window, and map it back to global
                            local_peak = np.argmax(windowed_corr)
                            peak_idx = local_peak + window_start
                            
                            # True sub-pixel interpolation
                            if window_start < peak_idx < window_end - 1:
                            ...
                            pixel_shift = (peak_idx + sub_pixel_offset) - center_idx 
                            pixel_ang_res = np.radians(FOV_DEG) / N_VISION
                            
                            raw_offset_th = -pixel_shift * pixel_ang_res 
                            
                            # SAFEGUARD
                            lc_offset_th = float(np.clip(raw_offset_th, -0.30, 0.30))
                            
                            # =================================================================
                            # 🌟 STEP 2: SOLVE TRANSLATION (Damped Least Squares / Ridge)
                            # =================================================================
                            curr_tof = np.array(tof_t)
                            mem_tof = np.array(node_tof_hits[matched_node])
                            
                            ray_angles = tof_angles + lc_offset_th
                            A = np.stack([np.cos(ray_angles), np.sin(ray_angles)], axis=1)
                            delta_d = mem_tof - curr_tof
                            
                            valid_mask = (curr_tof < 7.4) & (mem_tof < 7.4)
                            
                            # 🌟 THE FIX: Wrap the entire execution in this gate!
                            if np.sum(valid_mask) >= 2:
                                A_valid = A[valid_mask]
                                delta_d_valid = delta_d[valid_mask]
                                
                                # Damped Least Squares (Tikhonov Regularization)
                                lambda_damp = 0.05  
                                AtA = A_valid.T @ A_valid
                                Atb = A_valid.T @ delta_d_valid
                                ...
                                # cth = drifted current heading
                                # matched_th = perfectly relaxed anchor heading
                                # lc_offset_th = the physical crossing angle difference
